...
| [`esp32c6-cycles`](esp32c6-cycles) | **K3's cycle rows on the part the clause NAMES**: the twin of `xiao-s3-cycles`, on RV32 silicon where `mcycle` is architectural rather than a QEMU work count. **BUILDS, NEVER RUN** -- no C6 has been on this bench, so it carries no numbers. Exists so the clause waits on hardware alone, not on hardware and then a day of bring-up | an ESP32-C6 on a serial port. Builds on STABLE -- no esp toolchain, no build-std, unlike every Xtensa cell |
